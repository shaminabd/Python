...
D = a1 * (b2 * c3 - b3 * c2) - b1 * (a2 * c3 - a3 * c2) + c1 * (a2 * b3 -
                                                                a3 * b2)
if D == 0:
    print(
        "The determinant of the coefficient matrix is zero, Cramer's Rule cannot be applied"
    )
else:
    x = (d1 * (b2 * c3 - c2 * b3) - b1 * (d2 * c3 - d3 * c2) + c1 *
         (d2 * b3 - b2 * d2)) / D
    y = (a1 * (d2 * c3 - d3 * c2) - d1 * (a2 * c3 - c2 * a3) + c1 *
         (a2 * d3 - d2 * a3)) / D
    z = (a1 * (b2 * d3 - d2 * b3) - b1 * (a2 * d3 - d2 * a3) + d1 *
         (a2 * b3 - a3 * b2)) / D
